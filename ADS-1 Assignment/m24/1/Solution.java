...
class Solution {
	...
	public static void main(String[] args) {
		...
		for (i = 0; i < n; i++) {
			...
			Student stdObj = new Student(tokens[1],
		...
		for (j = 0; j < m; j++) {
			...
			if (queryTokens[0].equals("get")) {
				if (Integer.parseInt(queryTokens[2]) == 1) {
					if (schObj.get(Integer.parseInt(
					                   queryTokens[1])) == null) {
						System.out.println("Student doesn't exists...");
					} else {
						System.out.println(schObj.get(Integer.parseInt(
						                                  queryTokens[1])).getstudentname());

						if (schObj.get(Integer.parseInt(queryTokens[1])) == null) {
							System.out.println("Student doesn't exists...");
						} else {
							System.out.println(schObj.get(
							                       Integer.parseInt(
							                           queryTokens[1])).gettotalmarks());
						}
					}
				}
			}
